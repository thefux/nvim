...
function! SendDataToTimeTrackService(...)
    ...
    let l:state = get(a:, 3, '')
    call rpcnotify(s:timetrackId, s:TimeTrack, s:StoreInFile, l:command, l:message, l:state)
endfunction


function! s:TerminateTimeTrack()
    call jobstop(s:timetrackId)
    " call rpcnotify(s:timetrackId, s:Terminate)
endfunction
